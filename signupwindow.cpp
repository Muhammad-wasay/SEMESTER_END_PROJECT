...
void signupwindow::on_createbtn_clicked()
{
    ...
    if(username.isEmpty()|| email.isEmpty()|| pin.isEmpty()|| confirm_pin.isEmpty()|| accountType.isEmpty())
    {
        QMessageBox::critical(this, "Empty Field", "Field(s) is(are) Empty");
        return;
    }
    if(!check || initialbalance<0)
    {
    ...
    QString generatedAccountNumber = generateAccountNumber();

    // Set the generated account number in the UI (optional)
    ui->accountnumber->setText(generatedAccountNumber);
   ...
   query.bindValue(":account_number", generatedAccountNumber);
    ...
    central->setUserDetails(username, email, accountType, initialbalance, generatedAccountNumber);
    central->show();
}
