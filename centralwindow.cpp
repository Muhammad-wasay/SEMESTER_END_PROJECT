...
Centralwindow::~Centralwindow() {
    delete ui;
    delete deposit_dia;    // Safely delete to avoid memory leaks
    delete withdraw_dia;
    delete transfer_dia;
    delete transaction_win;
}
        ...
        if (depositAmount > 0)
        {
            ...
            */
            ...
            if (userAccountType.toLower().contains("saving")) {
                ...
                QMessageBox::information(this, "Deposit with Interest", "Interest of 3% applied. Total added: " + QString::number(totalWithInterest, 'f', 2));
            } else {
...
void Centralwindow::on_withdrawbtn_clicked()
{
    ...
    if (withdraw_dia.exec() == QDialog::Accepted) // Show modal deposit dialog
    {
        ...
        {
            double fee = 25;
            double totalWithdrawalAmount = withdrawAmount + fee;  // Adding fee to withdrawal amount

            // Check if user has sufficient funds including fee
            if (totalWithdrawalAmount <= currentBalance)
            {
                double newBalance = currentBalance - totalWithdrawalAmount;  // Deduct total (amount + fee)
            ...
            if (query.exec())
            {
                QMessageBox::information(this, "Withdrawal Successful", "Your balance has been updated with fees deduction of 25Pkr.");
                QSqlQuery query;
                query.prepare("INSERT INTO transactions (user_email, type, amount) VALUES (:email, :type, :amount)");
                query.bindValue(":email", userEmail);
            ...
            {
                QMessageBox::warning(this, "Insufficient Funds", "You do not have enough balance to cover the withdrawal and fee.");
            }
        }
        else
        {
